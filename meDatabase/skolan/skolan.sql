...
--
-- Host: MAC-KILLER-PRO.local    Database: skolan
-- ------------------------------------------------------
...
INSERT INTO `kurstillfalle` VALUES (2,'SVT101','gyl',1),(3,'SVT101','gyl',3),(4,'SVT201','ala',1),(5,'SVT202','ala',2),(6,'SVT401','sna',1),(7,'KVA101','hoc',1),(8,'DJU101','hag',3),(9,'DRY101','sna',2),(10,'DRY102','sna',3),(11,'MUG101','min',4);
...
/*!50001 VIEW `v_larare` AS select `larare`.`akronym` AS `akronym`,`larare`.`avdelning` AS `avdelning`,`larare`.`fornamn` AS `fornamn`,`larare`.`efternamn` AS `efternamn`,`larare`.`kon` AS `kon`,`larare`.`lon` AS `lon`,`larare`.`fodd` AS `fodd`,`larare`.`kompetens` AS `kompetens`,timestampdiff(YEAR,`larare`.`fodd`,curdate()) - 6 AS `alder` from `larare` */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`dbadm`@`%` SQL SECURITY DEFINER */
/*!50001 VIEW `v_lonerevision` AS select `l`.`akronym` AS `akronym`,`l`.`fornamn` AS `fornamn`,`l`.`efternamn` AS `efternamn`,`p`.`lon` AS `pre`,`l`.`lon` AS `nu`,`l`.`lon` - `p`.`lon` AS `diff`,`p`.`kompetens` AS `prekomp`,`l`.`kompetens` AS `nukomp`,`l`.`kompetens` - `p`.`kompetens` AS `diffkomp`,round((`l`.`lon` - `p`.`lon`) / `p`.`lon` * 100,2) AS `proc`,case when round((`l`.`lon` - `p`.`lon`) / `p`.`lon` * 100,2) > 3 then 'ok' else 'nok' end AS `mini` from (`larare` `l` join `larare_pre` `p` on(`l`.`akronym` = `p`.`akronym`)) order by `l`.`akronym` desc */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`dbadm`@`%` SQL SECURITY DEFINER */
/*!50001 VIEW `v_namn_alder` AS select concat(`larare`.`fornamn`,' ',`larare`.`efternamn`,' (',lcase(`larare`.`avdelning`),')') AS `namn`,timestampdiff(YEAR,`larare`.`fodd`,curdate()) - 6 AS `alder` from `larare` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2024-02-23 14:35:50
